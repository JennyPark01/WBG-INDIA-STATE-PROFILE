  ...
  dashboardPage(
    ...
    dashboardSidebar(
      sidebarMenu(
        ...
        menuItem("Fiscal Profile", icon=icon("coins"),
...
server <- function(input, output, session) {
  ...
  output$fiscaltool_table <- renderUI({
    ...
    # For a given state+code, compute within the year range
    ...
    for (rdef in fiscaltool_row_defs) {
      ...
      # State summary column (based on calculation mode)
  ...
  # Extracts a single code's value for a state+year from pfr_2_1_data
  ...
  # Compute group average for a code across a set of states (last available in year range)
  ...
  # Render Expenditure table (reuses same pattern as Revenues)
  ...
  # Chart components for expenditure (matching Excel 3_1 Chart 0 SERIES exactly)
  # Excel uses: row 11 (cur_total), row 15 (cur_nondev), row 23 (cap_outlay), row 28 (loans)
  # Excel overlaps cur_nondev inside cur_total. In Plotly we replicate this by:
  #   bottom segment = cur_total - cur_nondev (= cur_dev, shown as dark navy)
  #   next segment   = cur_nondev (lighter blue)
  # Together they visually equal cur_total, and the stacking is correct.
  exp_chart_components <- list(
    list(code = "cur_total",  code2 = "cur_nondev", label = "Total current expenditure", mode = "subtract"),
    list(code = "cur_nondev", code2 = NA,           label = "Current non-developmental expenditure", mode = "direct"),
    list(code = "cap_outlay", code2 = NA,           label = "Total Capital Outlay", mode = "direct"),
    list(code = "loans",      code2 = NA,           label = "Loans and Advances", mode = "direct")
  )
  ...
  output$fhi_quality_chart <- plotly::renderPlotly({
    ...
    # Min-max normalization scaled to 0-100 (matching Excel 4_1 formula)
  ...
  # Download for Quality of Expenditure / Fiscal Health Index
