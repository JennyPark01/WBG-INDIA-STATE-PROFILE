...
# PFR display name mapping: CSV state name -> nice display name
pfr_display_map <- c(
  "NCT Delhi"          = "Delhi",
  "Jammu and Kashmir"  = "Jammu & Kashmir"
)

# Build dropdown: value = CSV state name, display = nice name
      ...
      tabItems(
        ...
        tabItem(
          ...
          tags$h2("Latest Snapshot",
                  style = "font-weight:bold; color:navy; margin-bottom:4px;"),

          # --- Sub-tab switcher ---
          tabsetPanel(
            id = "tab1_sub", type = "pills",

            # ============ SUB-TAB 1: Latest Snapshot ============
            tabPanel(
              "Latest Snapshot", value = "snapshot",
          ...
          fluidRow(
            ...
            ), # end tabPanel "Latest Snapshot"

            # ============ SUB-TAB 2: Fiscal Profile ============
            tabPanel(
              "Fiscal Profile", value = "fiscal",
          ...
          fluidRow(
            ...
            ), # end tabPanel "Fiscal Profile"

            # ============ SUB-TAB 3: Fiscal Tool (Excel-like) ============
            tabPanel(
              "Fiscal Tool", value = "fiscal_tool",
...
server <- function(input, output, session) {
  ...
  output$fiscal_tile_gdppc <- renderValueBox({
    ...
    txt <- if (is.na(info$val)) "N/A" else paste0("\u20B9", formatC(info$val, format = "f", digits = 1), "K")
    sub <- if (is.na(info$yr)) "GDP per capita" else paste0("GDP per capita (", info$yr, ")")
  ...
  # --- Download handler for Fiscal Profile ---
  output$download_fiscal <- downloadHandler(
    filename = function() {
      paste0("Fiscal_Profile_", input$fiscal_state, "_", Sys.Date(), ".csv")
    },
    content = function(file) {
      df <- fiscal_filtered()
      wide <- df %>%
  ...
  output$fiscaltool_table <- renderUI({
    req(input$fiscaltool_state)
    df <- pfr_data %>% dplyr::filter(State == input$fiscaltool_state)
    ...
    # State display name
    state_display <- input$fiscaltool_state
    ...
    make_sparkline <- function(values, years) {
      # Extract non-NA values with their positions
      valid <- !is.na(values)
      ...
      # Color: compare first vs last value
    ...
    # "Last" column = latest value for the state
    header_html <- paste0(
      '<tr>',
      ...
      '<th style="background:#1a5276; color:white; padding:8px 8px; text-align:center;
          font-size:12px; min-width:80px; position:sticky; top:0;">', state_display, '</th>',
      '</tr>'
    )
    ...
    for (rdef in fiscaltool_row_defs) {
      if (rdef$type == "blank") {
        body_rows <- paste0(body_rows,
          '<tr><td colspan="', length(all_years) + 3,
      ...
      # Sparkline: collect values for all years
      ...
      for (yr in all_years) {
        ...
        if (!is.null(v) && !is.na(v)) {
          ...
          cell_text <- formatC(v, format = "f", digits = 2)
      ...
      # "State" summary column = last available value
      if (!is.na(last_val)) {
        lv_color <- if (last_val < 0) "color:#c0392b;" else "color:#003366;"
        lv_text <- formatC(last_val, format = "f", digits = 2)
        row_html <- paste0(row_html,
          '<td style="padding:5px 8px; text-align:right; font-weight:bold; font-size:12px;',
          ' background:#eaf2f8; ', lv_color, ' font-family:Consolas,monospace;">', lv_text, '</td>')
      } else {
        row_html <- paste0(row_html,
          '<td style="padding:5px 8px; text-align:center; background:#eaf2f8; color:#ccc;">&mdash;</td>')
      }

      # Alternating row background
      bg <- ""
      body_rows <- paste0(body_rows, '<tr style="', bg, '">', row_html, '</tr>')
    }
  ...
  # Download for Fiscal Tool
  output$download_fiscaltool <- downloadHandler(
    filename = function() {
      paste0("Fiscal_Tool_", input$fiscaltool_state, "_", Sys.Date(), ".csv")
    },
    content = function(file) {
      df <- pfr_data %>% dplyr::filter(State == input$fiscaltool_state)
      wide <- df %>%
